...
def quaternion_to_rotation_matrix(quaternion):
    """
    Convert a quaternion rotation to a 3D rotation matrix using scipy.

    Parameters:
    - quaternion (list or np.array): Quaternion rotation in the form [w, x, y, z].
...
def compute_gt_rmat_colmap_sift(q1, q2, batch_size):
    gt_rmat_matrix = []
    # gt_mtx1 = []
    # gt_mtx2 = []
    for i in range(batch_size): 
        q1_i_vec = [q1[1][i].item(),q1[2][i].item(),q1[3][i].item(),q1[0][i].item()]
        q2_i_vec = [q2[1][i].item(),q2[2][i].item(),q2[3][i].item(),q2[0][i].item()]
        mtx1= quaternion_to_rotation_matrix(q1_i_vec)
        mtx2= quaternion_to_rotation_matrix(q2_i_vec)
        rmat = np.array(mtx2)@np.array(mtx1).transpose()
        gt_rmat_matrix.append(rmat)
        # gt_mtx1.append(mtx1)
        # gt_mtx2.append(mtx2)
    # gt_mtx1 = torch.tensor(gt_mtx1)
    # gt_mtx2 = torch.tensor(gt_mtx2)
    # gt_rmat_matrix = compute_rotation_matrix_from_two_matrices(gt_mtx1, gt_mtx2)
    gt_rmat_matrix = torch.tensor(np.array(gt_rmat_matrix))
    return gt_rmat_matrix.cuda()    
...
#    euler_angles = torch.cat((torch.zeros(rotation_x.size()).to(rotation_x).unsqueeze(1),
#                            rotation_y.float().unsqueeze(1),
#                            rotation_x.float().unsqueeze(1)),dim=1)
#    matrix = euler_angles_to_matrix(euler_angles.squeeze(),'ZYX')
#    return matrix
def compute_rotation_matrix_from_viewpoint(rotation_x, rotation_y, batch):
